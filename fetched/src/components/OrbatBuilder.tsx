...
function MilSymbol({ unitType, echelon, size = 56 }: { unitType: string; echelon: string; size?: number }) {
  ...
  useEffect(() => {
    ...
    loadMilsymbol().then(MS => {
      ...
      try {
        const sym = new MS.Symbol(sidc, { size, standard: "APP6", fillOpacity: 0, monoColor: "#d1d5db", outlineWidth: 0 });
        setSvg(sym.asSVG());
      } catch {
        try {
          const sym = new MS.Symbol(UNIT_SIDC_MAP[unitType] ?? UNIT_SIDC_MAP.infantry, { size, standard: "APP6", fillOpacity: 0, monoColor: "#d1d5db", outlineWidth: 0 });
          setSvg(sym.asSVG());
        } catch { setSvg(""); }
      }
    }).catch(() => setSvg(""));
