      ...
      <div className="bg-card border border-destructive/50 rounded-lg w-full max-w-md mx-4 shadow-2xl">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-bold uppercase tracking-wider text-lg">Issue Ban</h2>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-destructive/10 border border-destructive/30 rounded p-4">
            <p className="font-sans text-sm text-muted-foreground">
              Banning <span className="font-bold text-foreground">{user.username}</span> ({user.email}) will immediately block their access to the portal. This action is logged.
            </p>
          </div>
          <div>
            ...
            <input
              ...
              placeholder="Describe the TOS violation..."
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={onClose} className="flex-1 px-4 py-2 border border-border rounded text-sm font-display font-bold uppercase tracking-wider hover:bg-secondary transition-colors">
              Cancel
            </button>
            <button
              onClick={() => onConfirm(reason.trim() || "TOS Violation")}
      ...
      <div className="bg-card border border-destructive/50 rounded-lg w-full max-w-md mx-4 shadow-2xl">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-bold uppercase tracking-wider text-lg">Confirm Deletion</h2>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground transition-colors">
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-destructive/10 border border-destructive/30 rounded p-4">
            <p className="font-sans text-sm text-muted-foreground">
              This permanently deletes the {type} <span className="font-bold text-foreground">{name}</span>. This cannot be undone.
            </p>
          </div>
          <div>
...
function AccountsTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter(u =>
      u.username.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || String(u.id).includes(q)
    );
  }, [users, search]);
  ...
  const banMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin", "users"] }); toast({ title: "Ban Issued", description: "User has been banned." }); setBanTarget(null); },
    onError: () => toast({ title: "Failed", description: "Could not ban user.", variant: "destructive" }),
  });

  const unbanMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/admin/users/${id}/unban`, { method: "PATCH" }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin", "users"] }); toast({ title: "Ban Lifted", description: "User access restored." }); },
    onError: () => toast({ title: "Failed", description: "Could not unban user.", variant: "destructive" }),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin", "users"] }); toast({ title: "Account Deleted", description: "The account has been permanently removed." }); setDeleteTarget(null); },
    onError: (err: any) => toast({ title: "Failed", description: err?.error || "Could not delete account.", variant: "destructive" }),
  });

  return (
    <div className="space-y-4">
      {banTarget && (
        <BanModal user={banTarget} onClose={() => setBanTarget(null)} onConfirm={reason => banMutation.mutate({ id: banTarget.id, reason })} />
      )}
      {deleteTarget && (
        <DeleteModal name={deleteTarget.username} type="account" onClose={() => setDeleteTarget(null)} onConfirm={() => deleteMutation.mutate(deleteTarget.id)} />
      )}
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {filtered.map(user => (
            <div key={user.id} className={`bg-card border rounded overflow-hidden transition-colors ${user.status === "banned" ? "border-destructive/40" : user.status === "suspended" ? "border-amber-500/40" : "border-border"}`}>
              {/* Row Header */}
              <div
                className="flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-secondary/30 transition-colors select-none"
              ...
              {expanded === user.id && (
                <div className="border-t border-border bg-secondary/10 px-4 py-4 space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-sans">
                    ...
                    {user.status === "banned" && (
                      <div><p className="text-muted-foreground mb-1 font-display font-bold uppercase tracking-wider">Ban Reason</p><p className="text-destructive">{user.banReason || "—"}</p></div>
                    )}
                  </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16 text-muted-foreground font-sans text-sm">No accounts match your search.</div>
          )}
        </div>
      )}
    </div>
...
function GroupsTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return groups.filter(g =>
      g.name.toLowerCase().includes(q) || g.slug.toLowerCase().includes(q) || (g.owner_username || "").toLowerCase().includes(q)
    );
  }, [groups, search]);
  ...
  return (
    <div className="space-y-4">
      {deleteTarget && (
        <DeleteModal name={deleteTarget.name} type="group" onClose={() => setDeleteTarget(null)} onConfirm={() => deleteMutation.mutate(deleteTarget.id)} />
      )}
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {filtered.map(group => (
            <div key={group.id} className={`bg-card border rounded overflow-hidden transition-colors ${group.status === "rejected" ? "border-destructive/40" : group.status === "pending" ? "border-amber-500/40" : "border-border"}`}>
              <div
                className="flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-secondary/30 transition-colors select-none"
              ...
              {expanded === group.id && (
                <div className="border-t border-border bg-secondary/10 px-4 py-4 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <select
                      value={group.status}
                      onChange={e => statusMutation.mutate({ id: group.id, status: e.target.value })}
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16 text-muted-foreground font-sans text-sm">No groups match your search.</div>
          )}
        </div>
      )}
    </div>
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center gap-4 border-b border-border pb-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-sans text-sm">Administrative control over accounts, groups, and platform enforcement.</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 bg-destructive/10 border border-destructive/30 rounded p-4">
          ...
          <p className="text-sm font-sans text-muted-foreground">
            Actions taken here are <span className="text-foreground font-semibold">permanent and irreversible</span>. Bans are logged with reason and timestamp. Deletions cannot be undone.
          </p>
        </div>
