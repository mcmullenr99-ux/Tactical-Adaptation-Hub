...
router.get("/milsim-groups/:id/ribbons", requireAuth, async (req, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  const result = await db.execute(rawSql`
    SELECT r.*, 
      COALESCE(json_agg(json_build_object('id', g.id, 'roster_entry_id', g.roster_entry_id, 'awarded_by', g.awarded_by, 'awarded_at', g.awarded_at, 'callsign', ro.callsign)) FILTER (WHERE g.id IS NOT NULL), '[]') AS grants
    FROM milsim_ribbons r
    LEFT JOIN milsim_ribbon_grants g ON g.ribbon_id = r.id
    LEFT JOIN milsim_roster ro ON ro.id = g.roster_entry_id
    WHERE r.group_id = ${id}
    GROUP BY r.id
...
router.post("/milsim-groups/:id/ribbons", requireAuth, async (req, res): Promise<void> => {
  const userId = (req as any).user.id;
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  const [group] = await db.select().from(milsimGroupsTable).where(eq(milsimGroupsTable.id, id));
  if (!group || group.ownerId !== userId) { res.status(403).json({ error: "Forbidden" }); return; }
  const { name, description, design } = req.body as any;
  if (!name || !design) { res.status(400).json({ error: "name and design required" }); return; }
  const result = await db.execute(rawSql`
    INSERT INTO milsim_ribbons (group_id, name, description, design)
    VALUES (${id}, ${name}, ${description ?? null}, ${JSON.stringify(design)})
    RETURNING *
  `);
  res.status(201).json(result.rows[0]);
});

router.patch("/milsim-groups/:id/ribbons/:rid", requireAuth, async (req, res): Promise<void> => {
  const userId = (req as any).user.id;
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  ...
  const { name, description, design } = req.body as any;
  await db.execute(rawSql`
    UPDATE milsim_ribbons SET
      name = COALESCE(${name ?? null}, name),
      description = COALESCE(${description ?? null}, description),
      design = COALESCE(${design ? JSON.stringify(design) : null}::jsonb, design)
    WHERE id = ${rid} AND group_id = ${id}
  `);
  const result = await db.execute(rawSql`SELECT * FROM milsim_ribbons WHERE id = ${rid}`);
  res.json(result.rows[0]);
});

router.delete("/milsim-groups/:id/ribbons/:rid", requireAuth, async (req, res): Promise<void> => {
  const userId = (req as any).user.id;
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  const rid = parseInt(Array.isArray(req.params.rid) ? req.params.rid[0] : req.params.rid, 10);
  const [group] = await db.select().from(milsimGroupsTable).where(eq(milsimGroupsTable.id, id));
  if (!group || group.ownerId !== userId) { res.status(403).json({ error: "Forbidden" }); return; }
  await db.execute(rawSql`DELETE FROM milsim_ribbons WHERE id = ${rid} AND group_id = ${id}`);
  res.sendStatus(204);
});

router.post("/milsim-groups/:id/ribbons/:rid/grant", requireAuth, async (req, res): Promise<void> => {
  const userId = (req as any).user.id;
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  const rid = parseInt(Array.isArray(req.params.rid) ? req.params.rid[0] : req.params.rid, 10);
  const [group] = await db.select().from(milsimGroupsTable).where(eq(milsimGroupsTable.id, id));
  ...
  const result = await db.execute(rawSql`
    INSERT INTO milsim_ribbon_grants (ribbon_id, roster_entry_id, awarded_by)
    VALUES (${rid}, ${rosterEntryId}, ${(req as any).user.username})
    ON CONFLICT (ribbon_id, roster_entry_id) DO NOTHING
    RETURNING *
  `);
  res.status(201).json(result.rows[0] ?? { already: true });
});

router.delete("/milsim-groups/:id/ribbons/:rid/grant/:grantId", requireAuth, async (req, res): Promise<void> => {
  const userId = (req as any).user.id;
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  const rid = parseInt(Array.isArray(req.params.rid) ? req.params.rid[0] : req.params.rid, 10);
  const grantId = parseInt(Array.isArray(req.params.grantId) ? req.params.grantId[0] : req.params.grantId, 10);
  const [group] = await db.select().from(milsimGroupsTable).where(eq(milsimGroupsTable.id, id));
  if (!group || group.ownerId !== userId) { res.status(403).json({ error: "Forbidden" }); return; }
  await db.execute(rawSql`
    DELETE FROM milsim_ribbon_grants WHERE id = ${grantId} AND ribbon_id = ${rid}
  `);
  res.sendStatus(204);
});

router.get("/milsim-groups/:id/roster/:rosterId/ribbons", requireAuth, async (req, res): Promise<void> => {
  const rosterId = parseInt(Array.isArray(req.params.rosterId) ? req.params.rosterId[0] : req.params.rosterId, 10);
  const result = await db.execute(rawSql`
    SELECT r.*, g.id AS grant_id, g.awarded_by, g.awarded_at AS grant_date
    FROM milsim_ribbon_grants g
    JOIN milsim_ribbons r ON r.id = g.ribbon_id
    WHERE g.roster_entry_id = ${rosterId}
    ORDER BY g.awarded_at DESC
  `);
  res.json(result.rows);
});
