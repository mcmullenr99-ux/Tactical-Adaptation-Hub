...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parts = url.pathname.replace(/^\/functions\/posts/, '').split('/').filter(Boolean);
    const method = req.method;

    // GET /functions/posts or /functions/posts?category=...&milsim_group_id=...
    if (method === 'GET' && parts.length === 0) {
      ...
      const allPosts = await base44.asServiceRole.entities.Post.list({ ...filter });
      ...
      return Response.json({ posts: paged, total: sorted.length });
    }

    // GET /functions/posts/:id
    ...
    if (method === 'POST' && parts.length === 0) {
      ...
      const post = await base44.asServiceRole.entities.Post.create({
        ...
        user_role: full.role,
        user_nationality: full.nationality ?? null,
        title: title ?? null,
        body: postBody,
        category: category ?? 'general',
        milsim_group_id: milsim_group_id ?? null,
    ...
    // PATCH /functions/posts/:id — update/pin
    if (method === 'PATCH' && parts.length === 1) {
      ...
      const updated = await base44.asServiceRole.entities.Post.update(parts[0], updates);
      return Response.json(updated);
    }

    // DELETE /functions/posts/:id
    ...
    // POST /functions/posts/:id/comments
    if (method === 'POST' && parts.length === 2 && parts[1] === 'comments') {
      ...
      await base44.asServiceRole.entities.Post.update(parts[0], {
    ...
    // DELETE /functions/posts/:postId/comments/:commentId
    if (method === 'DELETE' && parts.length === 3 && parts[1] === 'comments') {
      ...
      const isMod = ['staff', 'moderator', 'admin'].includes(full.role);
      if (comment.user_id !== full.id && !isMod) return Response.json({ error: 'Forbidden' }, { status: 403 });
